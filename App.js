...
let pcontainer = document.querySelector('.productscontainer');
productslist.map((e) => {

pcontainer.innerHTML += `<div class="product">
...
document.querySelectorAll(".addtocart").forEach((e) => {
    e.onclick = (event) => {
        let id = event.target.getAttribute("productid");
        let price = event.target.previousElementSibling.innerHTML;
        let image =
            event.target.parentElement.previousElementSibling.previousElementSibling.getAttribute(
                "src"
            );
        let title = event.target.parentElement.previousElementSibling.innerHTML;
